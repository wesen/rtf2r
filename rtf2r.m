	...
	//
		...
		for( ; index < count; ++index ) {
				struct {
						...
						uint16_t Ascent;
						uint16_t FontFamily;
						uint8_t  FontStyle; // bitfield, 0x1=bold 0x2=italic 0x4=underline 0x8=outline 0x10=shadow 0x20=condensed 0x40=extended
						 uint8_t  Unused;    // ?, maybe the above field has 16 bits, so bits might need to be swapped ..  
						uint16_t FonstSize;
						uint16_t Color[3];
				} __attribute__((packed)) rtfs;
